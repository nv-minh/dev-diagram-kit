...
| BPMN engine (shared, NOT per-feature) | `${CLAUDE_PLUGIN_ROOT:-.claude}/skills/bpmn/engine/` — `bpmn-build.mjs` + `bpmn-layout-{auto,elk}.mjs` + `bpmn-layout.mjs` + `bpmn-semcheck.mjs` + `_viewer_template.html` + `node_modules` (installed once; plugin mode: the hook auto-installs into `${CLAUDE_PLUGIN_DATA}`). Run: `node "${CLAUDE_PLUGIN_ROOT:-.claude}/skills/bpmn/engine/bpmn-build.mjs" --dir docs/{feature}/bpmn`. |
