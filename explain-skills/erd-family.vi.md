...
[English](erd-family.md) · **Tiếng Việt**
...
- `explain-skills/erd.vi.md` — `/erd` (Mermaid, nhúng thẳng vào tài liệu, cho BA/stakeholder đọc).
- `explain-skills/d2-erd.vi.md` — `/d2-erd` (D2, cùng nội dung `/erd` nhưng khác kiểu vẽ, file ảnh riêng).
- `explain-skills/dbdiagram.vi.md` — `/dbdiagram` (DBML, gần dev nhất, xuất SQL, chia sẻ trên dbdiagram.io).

Quy tắc chọn diagram đầy đủ (cho mọi loại sơ đồ, không chỉ dữ liệu) nằm ở:

- `explain-skills/diagram-selection.vi.md` — bàn chỉ đường chọn loại sơ đồ (bản cho người).
