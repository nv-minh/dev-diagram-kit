...
[English](activity-family.md) · **Tiếng Việt**
...
- `explain-skills/activity.vi.md` — `/activity` (Mermaid, nhúng thẳng vào tài liệu).
- `explain-skills/activity-swimlane.vi.md` — `/activity-swimlane` (PlantUML, phân làn thật, ⭐ mặc định cho đa vai trò).
- `explain-skills/d2-activity.vi.md` — `/d2-activity` (D2, dàn gọn hơn Mermaid khi nhiều nhánh, file đứng riêng).
- `explain-skills/bpmn.vi.md` — `/bpmn` (chuẩn quốc tế BPMN, đưa vào phần mềm quản lý quy trình).
