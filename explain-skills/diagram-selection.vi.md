...
[English](diagram-selection.md) · **Tiếng Việt**
...
Ranh giới quan trọng nhất trong họ này: **số vai trò tham gia**. Nhiều vai trò giao cắt → `/activity-swimlane` (nó giữ mỗi vai một "làn" thẳng cột). Muốn hiểu vì sao và xem so sánh đầy đủ, đọc `explain-skills/activity-family.vi.md`.
...
Ranh giới quan trọng: `/erd` và `/d2-erd` chỉ khác **kiểu vẽ** (không cái nào "đẹp hơn"); còn `/dbdiagram` khác về **độ chi tiết** (gần dev nhất). Muốn hiểu sâu và xem so sánh đầy đủ, đọc `explain-skills/erd-family.vi.md`.
...
- `explain-skills/activity-family.vi.md` — chọn giữa `/activity`, `/activity-swimlane`, `/d2-activity` (+ `/bpmn`) cho **sơ đồ quy trình**.
- `explain-skills/usecase-family.vi.md` — phân biệt `/usecase`, `/usecase-diagram`, `/userstory` (kịch bản chữ / toàn cảnh hình / chia việc backlog).
- `explain-skills/erd-family.vi.md` — chọn giữa `/erd`, `/d2-erd`, `/dbdiagram` cho **sơ đồ dữ liệu**.

**Các explainer từng lệnh (nhóm chỉ có một lệnh):**

- `explain-skills/sequence.vi.md` — `/sequence` (trao đổi nhiều bên theo thời gian).
- `explain-skills/state.vi.md` — `/state` (vòng đời trạng thái của một đối tượng).
- `explain-skills/usecase-diagram.vi.md` — `/usecase-diagram` (bức tranh tổng actor + ca sử dụng).
- `explain-skills/d2-architect.vi.md` — `/d2-architect` (sơ đồ kiến trúc hệ thống).
- `explain-skills/bpmn.vi.md` — `/bpmn` (chuẩn quốc tế BPMN, mở bằng phần mềm quản lý quy trình).
