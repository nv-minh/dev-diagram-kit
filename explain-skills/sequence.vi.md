...
[English](sequence.md) · **Tiếng Việt**
...
- `explain-skills/state.vi.md` — vẽ **vòng đời trạng thái** của một đối tượng (đơn hàng: chờ → đã trả → đã giao). Khác `/sequence`: sơ đồ trạng thái vẽ *một đối tượng đi qua các trạng thái nào*, còn `/sequence` vẽ *nhiều bên trao đổi với nhau theo thời gian*.
- `explain-skills/activity-family.vi.md` — nhóm lệnh vẽ **sơ đồ quy trình** (từng bước một việc chạy thế nào, ai làm bước nào).
