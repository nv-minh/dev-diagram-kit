...
[English](activity.md) · **Tiếng Việt**
...
- **`explain-skills/activity-swimlane.vi.md`** — vẽ "phân làn" thẳng cột cho từng vai trò, là lựa chọn tốt hơn khi quy trình có nhiều vai trò tương tác chằng chịt (chính là chỗ `/activity` bị xô lệch).
- **`explain-skills/d2-activity.vi.md`** — vẽ sơ đồ ĐẸP đứng riêng (thành file ảnh) để đưa cho sếp/khách hàng xem hoặc xuất báo cáo, khi không cần nhúng thẳng trong tài liệu.
- **`explain-skills/activity-family.vi.md`** — bảng so sánh cả 3 công cụ trên, giúp bạn chọn nhanh nên dùng cái nào.
