...
- `explain-skills/activity-family.vi.md` — nhóm lệnh vẽ **sơ đồ quy trình** (từng bước một việc chạy thế nào). Khác `/state`: sơ đồ quy trình vẽ *dòng chảy công việc*, còn `/state` vẽ *vòng đời trạng thái của một đối tượng*.
