...
- `explain-skills/activity-swimlane.vi.md` — `/activity-swimlane`: cũng phân làn theo vai trò nhưng **nhẹ hơn**, để mô tả nghiệp vụ trong tài liệu (không cần chuẩn OMG / phần mềm quy trình). Đây là lựa chọn mặc định cho quy trình đa vai trò thường ngày.
- `explain-skills/activity.vi.md` — `/activity`: vẽ nhanh bằng Mermaid, nhúng thẳng vào tài liệu, hợp quy trình gọn.
- `explain-skills/d2-activity.vi.md` — `/d2-activity`: cách vẽ thứ ba, dàn gọn hơn Mermaid khi nhiều nhánh, cho file ảnh đứng riêng.
- `explain-skills/activity-family.vi.md` — bảng so sánh tổng hợp, giúp chọn đúng lệnh cho từng tình huống.
