...
[English](dbdiagram.md) · **Tiếng Việt**
...
- `explain-skills/erd.vi.md` — vẽ sơ đồ dữ liệu **nhúng thẳng vào tài liệu** để GitHub/Obsidian tự hiện (nhẹ, cho BA đọc).
- `explain-skills/d2-erd.vi.md` — **cùng nội dung như `/erd`, chỉ khác kiểu vẽ** (dùng công cụ D2 thay Mermaid), cho ra một file hình riêng.

Các lệnh vẽ sơ đồ khác trong cùng bộ công cụ:

- `explain-skills/sequence.vi.md` — vẽ **cuộc trao đổi giữa nhiều bên theo thời gian**.
- `explain-skills/state.vi.md` — vẽ **vòng đời trạng thái** của một đối tượng.

Chọn loại sơ đồ:

- `explain-skills/erd-family.vi.md` — so sánh nhanh cả 3 lệnh vẽ dữ liệu (`/erd`, `/d2-erd`, `/dbdiagram`), giúp chọn đúng cái cần.
- `explain-skills/diagram-selection.vi.md` — bàn chỉ đường cho **mọi loại sơ đồ** (khi chưa biết cần loại nào).
