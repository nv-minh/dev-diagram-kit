...
| Skill | Output | IDs minted | Use for |
|---|---|---|---|
| `/brainstorm` | Idea exploration doc with decisions + Open Questions | OQs | A raw idea, nothing structured yet — the root of the chain |
| `/urd` | User Requirements Document — personas, context of use, needs | `UN-` | Who the users are and what they need |
| `/brd` | Business Requirements Document — objectives, scope, cost-benefit, risks | `BO-` | The business case: why this, measured how |
| `/prd-epic` | Feature PRD — capabilities P0/P1/P2, goals/non-goals, release plan | `CAP-` | What we'll build for ONE feature |
| `/prd` | Product PRD (singleton, `docs/_product/`) — pitch, themes, Feature Map | — | Defining the WHOLE product |
| `/roadmap` | Roadmap (singleton) — RICE-lite scores, Now/Next/Later, dependencies | — | Sequencing and prioritizing features |
| `/srs` | SRS — testable FRs, NFRs, business rules, error matrix + diagram menu | `FR- NFR- BR- E-` | Precise system behavior, the source for everything downstream |

### Documents — specification

| Skill | Output | IDs minted | Use for |
|---|---|---|---|
| `/usecase` | Fully-dressed Cockburn use cases + index (the per-feature traceability matrix) | `UC-` | Actor-goal narratives with extensions; discovery or downstream of the SRS |
| `/userstory` | INVEST stories sliced from FRs + story index (status/priority/jira-key) | `US-` | Dev-ready backlog items; needs the SRS |
...
### Documents — testing & traceability

| Skill | Output | IDs minted | Use for |
|---|---|---|---|
| `/test-checklist` | Test-coverage outline (categorized: functional/boundary/error/NFR) | `CHK-` | The outline `/test-cases` expands; needs the SRS |
| `/test-cases` | Full test cases (steps/data/expected) | `TC-` | Expands each `CHK-`; refuses without a checklist |
| `/gap` | Cross-doc traceability matrix + coverage report | — | Proves the spine (UN→…→TC) is complete; read-only |
| `/cr` | Change Request (Impact Matrix + Rollback + guided apply) | `CR-` | Records a scope change + applies it as per-doc L2 diffs |
| `/reverse-doc` | Reconstructed BA docs from legacy sources (docx/pdf/code) | — | Brownfield docs with 3-level confidence (✅/🔵/🟡) |

### Documents — API integration (7-step chain)

| Skill | Output | Step | Use for |
|---|---|---|---|
| `/api-assess` | Build-vs-buy / provider scorecard | [0] | Choosing a provider (skippable when fixed) |
| `/api-doc` | Internal summary of THEIR contract | [1] | Digesting the OpenAPI/docs (refuses to fabricate) |
| `/api-design` | Integration Blueprint (orchestration/webhook/retry) | [2] | How WE orchestrate it |
| `/api-map` | 3-layer field mapping (provider ↔ model ↔ UI) | [3] | Per-field source-of-truth |
| `/api-checklist` | Integration test outline (test_layer + direction) | [4] | What to test across the integration |
| `/api-test` | Bruno collection + test table | [5] | Proving the calls work |
| `/api-readiness` | Go-live gate (cutover/flags/monitoring/rollback) | [6] | The go/no-go before production |

### Documents — delivery & sync

| Skill | Output | Use for |
|---|---|---|
| `/jira` | Push/sync stories ↔ Jira issues (keys into the story index) | External-write HITL; refuses stale stories |
| `/confluence` | Publish vault docs as a Confluence page tree | External-write HITL (for code→existing page use `/sync-confluence`) |
| `/export` | Stakeholder package (md/html/pdf/docx) + change history | A dated snapshot for stakeholders |
| `/userguide` | End-user manual (entry `.html` + bundle), light mode | Task-oriented guidance users read |
| `/meeting` | Meeting notes (decisions/blockers/actions as in-file tables) | Structured minutes, one self-contained file |
| `/inbox` | Zero-friction capture + triage routing via `/ba` | A parking lot for raw notes |
| `/doc-review` | Multi-agent doc quality review + apply accepted fixes | Renamed from `/review` (ecosystem collision) |
| `/dashboard` | One-file HTML vault status + regenerates feature-list | The internal pulse (statuses, staleness, OQ debt) |
...
A *fictional, anonymized* reinsurance underwriting platform (modelled on a real NestJS + React codebase — no real names/fields/paths). **Every diagram skill ships one rendered example**, generated by its real pipeline. Document skills cover the discovery→spec→test spine plus an API digest→blueprint→map trio; remaining API/delivery skills are guide-only (see the status tables in [`example/atlas-re/README.md`](example/atlas-re/README.md)). Domain notes: [`DOMAIN.md`](example/atlas-re/DOMAIN.md).
